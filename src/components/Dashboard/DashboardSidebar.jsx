                        ...
                        return (
                            <Link
                                ...
                                className={`flex items-center ${isCollapsed ? 'justify-center' : ''} px-4 py-3 rounded-xl transition-all duration-200 group ${isActive
                                        ? 'bg-linear-to-r from-[#ff7400] to-orange-500 text-white'
                                        : 'hover:bg-gray-50 text-gray-700'
                                    }`}
                            >
                                <div className="flex items-center">
                                    <Icon className={`h-5 w-5 ${isCollapsed ? '' : 'mr-3'} ${isActive ? 'text-white' : 'text-gray-500 group-hover:text-[#ff7400]'
                                        }`} />
                                    {!isCollapsed && (
                    ...
                    {menuItems.map((item) => {
                        ...
                        return (
                            <Link
                                key={item.name}
                                href={item.href}
                                className={`flex flex-col items-center justify-center flex-1 p-2 ${isActive ? 'text-[#ff7400]' : 'text-gray-600'
                                    }`}
                            >
                                <div className={`relative ${isActive ? 'text-[#ff7400]' : ''}`}>
                                    <Icon className="h-5 w-5" />
                                    {isActive && (
                                        <div className="absolute -top-1 -right-1 w-2 h-2 bg-[#ff7400] rounded-full"></div>
                                    )}
                                </div>
